...
BeforeAll {
    $script:ScriptPath = Join-Path $PSScriptRoot '..\scripts\verify-maui.ps1'
}

Describe "verify-maui.ps1 - Script Validation" {
...
Describe "verify-maui.ps1 - Parameter Validation" {
    BeforeAll {
...
Describe "verify-maui.ps1 - Function Definitions" {
    BeforeAll {
...
Describe "verify-maui.ps1 - Content Checks" {
    BeforeAll {
    ...
    It "Contains MAUI workload check" {
        $script:Content | Should -Match 'maui'
    }

    It "Contains workload list check" {
        $script:Content | Should -Match 'dotnet workload list'
    }

    It "Contains MAUI Windows workload check" {
        $script:Content | Should -Match 'maui-windows'
    }

    It "Contains MAUI Android workload check" {
        $script:Content | Should -Match 'maui-android'
    }

    It "Contains Windows App SDK check" {
        $script:Content | Should -Match 'Windows App SDK'
    }
    ...
    It "Contains MAUI project creation test" {
        $script:Content | Should -Match 'dotnet new maui'
    }
    ...
    It "Includes capability identifier" {
        $script:Content | Should -Match 'capability.*=.*"maui"'
    }
}

Describe "verify-maui.ps1 - Execution Tests" {
    ...
    Context "When .NET SDK is available" {
        ...
        It "Should include capability identifier in JSON output" -Skip:(-not $script:DotnetAvailable) {
            $output = & $script:ScriptPath -JsonOutput 2>&1 | Out-String
            $json = $output | ConvertFrom-Json
            $json.capability | Should -Be 'maui'
        }
        ...
        It "Should accept MinimumVersion parameter" -Skip:(-not $script:DotnetAvailable) {
            { & $script:ScriptPath -MinimumVersion "8.0" -JsonOutput 2>&1 } | Should -Not -Throw
        }
...
Describe "verify-maui.ps1 - Security and Best Practices" {
    BeforeAll {
        $script:Content = Get-Content $script:ScriptPath -Raw
    }
...
Describe "verify-maui.ps1 - MAUI Specific Checks" {
    BeforeAll {
        $script:Content = Get-Content $script:ScriptPath -Raw
    }

    It "Tests MAUI workload installation" {
        $script:Content | Should -Match 'workload list'
    }

    It "Tests MAUI project template" {
        $script:Content | Should -Match 'dotnet new maui'
    }
    ...
    It "Checks for Windows App SDK" {
        $script:Content | Should -Match 'windowsappsdk|Windows App SDK'
    }
    ...
    It "Includes installation instructions for missing workloads" {
